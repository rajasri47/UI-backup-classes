...
var validateCharTypes = (event) => {
    ...
    if((event.keyCode >= 33 && event.keyCode <= 47) || (event.keyCode >= 58 && event.keyCode <= 64) ||(event.keyCode >= 91 && event.keyCode <= 96) || (event.keyCode >= 123 && event.keyCode <= 126)){
        totalSpecialChars++;
        if(totalSpecialChars == 1){
         document.querySelector("#splCharValidation").classList.remove('pwdErr')
         document.querySelector("#splCharValidation").classList.add('pwdValid')
     }else{
         document.querySelector("#splCharValidation").classList.add('pwdErr')
         document.querySelector("#splCharValidation").classList.remove('pwdValid')
